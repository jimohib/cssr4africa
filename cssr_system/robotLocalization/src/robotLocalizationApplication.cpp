...
RobotLocalizationNode::RobotLocalizationNode() : nh_("~"), it_(nh_), tf_buffer_(), tf_listener_(tf_buffer_) {
    ...
    if (enable_head_scanning_) {
        head_traj_pub_ = nh_.advertise<trajectory_msgs::JointTrajectory>("/pepper_dcm/Head_controller/command", 10);
        
        // Wait for connection
        ros::Duration(1.0).sleep();
        
        ROS_INFO("Head control setup:");
        ROS_INFO("  Using trajectory publisher: %s", head_traj_pub_.getTopic().c_str());
        ROS_INFO("  Head_controller subscribers: %u", head_traj_pub_.getNumSubscribers());
        
        if (head_traj_pub_.getNumSubscribers() == 0) {
            ROS_WARN("No subscribers to head trajectory topic! Head scanning may not work.");
        } else {
            ROS_INFO("Head trajectory publisher connected successfully");
        }
    }
    ...
    ROS_INFO("Head control method: TRAJECTORY (Pepper compatible)");
    ...
    if (enable_head_scanning_) {
        ...
        ROS_INFO("  Head joints tracked: HeadYaw, HeadPitch");
        ROS_INFO("  Head control topic: %s", head_traj_pub_.getTopic().c_str());
    }
    ROS_INFO("================================================");
