...
void RobotLocalizationNode::moveHeadToPosition(double yaw_angle) {
    ...
    std_msgs::Float64 head_cmd;
    head_cmd.data = yaw_angle;
    head_yaw_pub_.publish(head_cmd);
    
    if (verbose_) {
        ROS_INFO("Moving head to %.1f degrees", yaw_angle * 180.0/M_PI);
    }
}
...
bool RobotLocalizationNode::isHeadMovementComplete() {
    ...
    return yaw_error < 0.05; // 3 degrees tolerance
}
