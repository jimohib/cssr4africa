 ...
 *
 * This node is responsible for navigating the robot to a goal location in the environment map using a path planning algorithm.
 * The node reads the robot pose input and the goal location from the service request and navigates the robot to the goal location.
 * The node uses the path planning algorithm to find the shortest path to the goal location.
 *
 * Libraries
 * Standard libraries - std::string, std::vector, std::thread, std::fstream, std::cout, std::endl, std::cin, std::pow, std::sqrt, std::abs
 * ROS libraries - ros/ros.h, ros/package.h, actionlib/client/simple_action_client.h, control_msgs/FollowJointTrajectoryAction.h, geometry_msgs/Twist.h
