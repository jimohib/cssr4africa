/* robotLocalizationTestImplementation.cpp   Function definitions and implementation of the robot localization unit tests
*
...
void RobotLocalizationNode::printCopyrightMessage() {
    ...
    ROS_INFO("Transformation Research Grant Programme.");
    ROS_INFO("Website: www.cssr4africa.org");
    ROS_INFO("This program comes with ABSOLUTELY NO WARRANTY.");
    ROS_INFO("=======================================================");
}

void RobotLocalizationNode::printStartupMessage(const std::string& message) {
    ROS_INFO("robotLocalization: %s", message.c_str());
}

void RobotLocalizationNode::heartbeatCallback(const ros::TimerEvent& event) {
    ROS_INFO("robotLocalization: running");
    
    // Optional: Print additional status information in unit test mode
    if (unit_test_mode_) {
        ros::Duration uptime = ros::Time::now() - node_start_time_;
        ROS_INFO("robotLocalization: uptime %.1f seconds", uptime.toSec());
        
        // Print topic status
        if (!latest_image_.empty()) {
            ROS_INFO("robotLocalization: image data available (%dx%d)", 
                     latest_image_.cols, latest_image_.rows);
        } else {
            ROS_WARN("robotLocalization: no image data");
        }
        
        if (camera_info_received_) {
            ROS_INFO("robotLocalization: camera info received");
        } else {
            ROS_WARN("robotLocalization: camera info not received");
        }
        
        ROS_INFO("robotLocalization: landmarks loaded: %zu", landmarks3D_.size());
        ROS_INFO("robotLocalization: current pose: (%.3f, %.3f, %.3f°)", 
                 current_pose_.x, current_pose_.y, 
                 angles::to_degrees(current_pose_.theta));
    }
}
...
void RobotLocalizationNode::cameraInfoCallback(const sensor_msgs::CameraInfo::ConstPtr& msg) {
    ...
    // When camera info is received
    if (camera_info_received_ && unit_test_mode_) {
        printStartupMessage("camera intrinsics received and validated");
    }
}
...
void RobotLocalizationNode::odomCallback(const nav_msgs::Odometry::ConstPtr& msg) {
    ...
    static bool first_odom_logged = false;
    if (!first_odom_logged && unit_test_mode_) {
        printStartupMessage("first odometry message received");
        first_odom_logged = true;
    }
...
void RobotLocalizationNode::imageCallback(const sensor_msgs::Image::ConstPtr& msg) {
    // Print first image message received
    static bool first_image_logged = false;
    if (!first_image_logged && unit_test_mode_) {
        printStartupMessage("first image message received");
